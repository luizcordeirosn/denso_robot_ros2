...
def control_node(robot_controllers_path, bcap_slave_control_cycle_msec, sim):
    ...
    control_node = Node(
        ...
        parameters=[robot_controllers_path, denso_robot_control_parameters],
        remappings=[
...
def static_tf(child_frame, use_sim_time):
    ...
    static_tf = Node(
        ...
        output="log",
        arguments=["--frame-id", "world", "--child-frame-id", child_frame],
        parameters=[{"use_sim_time": use_sim_time}],
    )
...
def moveit_servo(moveit_config, sim, arm=None):
    ...
    servo_params = (
        ...
        .yaml(parameter_namespace="moveit_servo", file_path="config/moveit_servo.yaml")
        .parameter("moveit_servo.use_gazebo", sim)
    )

    if arm == "left":
        servo_params = (
            servo_params.parameter("moveit_servo.move_group_name", "left_arm")
            ...
            .parameter(
                ...
                "/left_denso_joint_trajectory_controller/joint_trajectory",
            )
        )
    elif arm == "right":
        servo_params = (
            servo_params.parameter("moveit_servo.move_group_name", "right_arm")
            .parameter("moveit_servo.planning_frame", "world")
            .parameter("moveit_servo.ee_frame_name", "right_J6")
            .parameter("moveit_servo.robot_link_command_frame", "right_base_link")
            .parameter(
                "moveit_servo.command_out_topic",
                "/right_denso_joint_trajectory_controller/joint_trajectory",
            )
        )
